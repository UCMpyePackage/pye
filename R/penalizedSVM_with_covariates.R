...
#' @param cost parameter of the SVM method. Default is 10
#' @param lambda1 the penalization parameter of the regressors X. Default is
#' NULL, in case the unpenalized methods are run
#' @param lambda2 the penalization parameter of the regressors X in case
#' ElasticSCAD (SCAD + L2). It is the parameter associated to the L2-norm.
#' Default is 0.05
#' @param scale paraemter in the original SVM methods. Default is FALSE
...
#' among "SCADSVM", "ElasticSCADSVM", "l1SVM" and "enSVM".
...
#' lambda <- 0.1
#'
#' z_hat <- all_svm_estimation(df=df, X=X, y=y, model_type=model_type, lambda1=lambda,
...
#'
#' @export
all_svm_estimation <- function (df, X=names(df[,!(names(df) == y)]), y="y", cost=10, lambda1=NULL, lambda2=0.05, alpha=0.5,
                                scale=FALSE, model_type, regressors_betas=NULL, fold=NULL, trace=1, seed = 1, max.print=10,...){
  ...
  if (length(lambda1)>1){stop("all_svm_estimation stopped because lambda1 needs to have length 1")}
  ...
  if (!all((levels(factor(df_y_as_matrix)) == c("-1", "1")))){
    ...
    #cat("->You passed me a 0/1 'y' variable, while I needed a -1/1 'y' variable.\n Don't worry, I knew it and I've already solved it.\n")
  }

  #soppress waning messages (if necessary)
  #options(warn=-1)
  #options(warn=1) #print the warning but do not stop the execution
  #options(warn=2)


  #model_type can be: "SVM_linear", "SVM_polynomial", "SVM_radial",
  #"SVM_sigmoid", "SCADSVM", "ElasticSCADSVM", "l1SVM", "enSVM"
  if (model_type=="SVM_linear"){

    #estimate the svm_linear model
    svm_model <- e1071::svm(x=df_x_as_matrix, y=df_y_as_matrix, cost=cost,
                            scale=scale, kernel="linear",
                            type="C-classification")

    #for some special character the output of svm() changes the name of the variables!!!! we solve the problem:
    colnames(svm_model$SV) <- colnames(df1[,names(df1) %in% X])

    z_hat = as.data.frame(svm_model$decision.values)
    ...
    #build betas_hat properly
    betas_hat=rep(0,length(X))
    names(betas_hat)=names(df1[,names(df1) %in% X])
    betas_hat[which(names(betas_hat) %in% attr((t(svm_model$coefs) %*% svm_model$SV)[1,], "names"))]=(t(svm_model$coefs) %*% svm_model$SV)[1,]
  ...
  } else if (model_type=="SVM_polynomial"){
    svm_model <- e1071::svm(x=df_x_as_matrix, y=df_y_as_matrix, cost=cost,
                            scale=scale, kernel="polynomial",
                            type="C-classification")
  ...
  } else if (model_type=="SVM_sigmoid"){
    svm_model <- e1071::svm(x=df_x_as_matrix, y=df_y_as_matrix, cost=cost,
                            scale=scale, kernel="sigmoid",
                            type="C-classification")
  ...
  } else if (model_type=="SCADSVM"){
    #scadsvc: for SCAD SVM
    tmp <- try( SCADSVM <- penalizedSVM::scadsvc(lambda1 = lambda1, x=df_x_as_matrix,
                                                 y=df_y_as_matrix,
                                                 a = 3.7, maxIter = 100000, verbose = FALSE,
                                                 seed=seed, tol = 10^(-8)), silent = TRUE)
    ...
    while(inherits(tmp, "try-error")){
      cat("Error number", count,"; \n")
      print(tmp)
      cat("I Added 0.000001 to lambda to overcome it! \n")
      lambda1_new = lambda1_new + 0.000001 #with this little modification it works and do not return error
      tmp <- try( SCADSVM <- penalizedSVM::scadsvc(lambda1 = lambda1_new, x=df_x_as_matrix,
                                                   y=df_y_as_matrix,
                                                   a = 3.7, maxIter = 100000, verbose = FALSE,
                                                   seed=seed, tol = 10^(-5)), silent = TRUE)
      count <- count+1
      if (count > 100) {
        if (!inherits(tmp, "try-error")){
          ...
          while(inherits(tmp, "try-error")){
            ...
            cat("I deleted one more column from the the dataframe to try to solve the error! \n")
            tmp <- try( SCADSVM <- penalizedSVM::scadsvc(lambda1 = lambda1, x=df_x_as_matrix[,column_start:ncol(df_x_as_matrix)],
                                                         y=df_y_as_matrix,
                                                         a = 3.7, maxIter = 100000, verbose = FALSE,
                                                         seed=seed, tol = 10^(-5)), silent = TRUE)
            count <- count+1
            df_x_as_matrix1 <- df_x_as_matrix[,column_start:ncol(df_x_as_matrix)]
            if (count > 200) {break}
          }
        }
      }
    ...
    SCADSVM <- penalizedSVM::scadsvc(lambda1 = lambda1_new, x=df_x_as_matrix1,
                                     y=df_y_as_matrix,
                                     a = 3.7, maxIter = 100000, verbose = FALSE,
                                     seed=seed, tol = 10^(-5))
    ...
    #since sometimes it fails I set the cutoff point. (I put the average of the identified points)
    #c <- sum(OptimalCutpoints::optimal.cutpoints(data= df_hat, X = "z_hat", status = "y", methods = "MCT", tag.healthy = 0,
    #                                             generalized.Youden=F)$MCT$Global$optimal.cutoff$cutoff)/
    #  length(OptimalCutpoints::optimal.cutpoints(data= df_hat, X = "z_hat", status = "y", methods = "MCT", tag.healthy = 0,
    #                                             generalized.Youden=F)$MCT$Global$optimal.cutoff$cutoff)

    #this model works fine, do not need to reset c
    c <- 0
    ...
    tmp <- try(ElasticSCADSVM <- penalizedSVM::scad_L2.svc(lambda1=lambda1, lambda2=lambda2,
                                                           x=df_x_as_matrix, y=df_y_as_matrix,
    ...
    while(inherits(tmp, "try-error")){
      ...
      cat("I Added 0.000001 to lambda to overcome it! \n")
      lambda1_new = lambda1_new + 0.000001 #with this little modification it works and do not return error
      tmp <- try( ElasticSCADSVM <- penalizedSVM::scad_L2.svc(lambda1 = lambda1_new, lambda2=lambda2,
                                                  x=df_x_as_matrix, y=df_y_as_matrix,
                                                  a = 3.7, maxIter = 100000,
      ...
      if (count > 100) {
        if (!inherits(tmp, "try-error")){
          ...
          while(inherits(tmp, "try-error")){
            ...
            cat("I deleted one more column from the the dataframe to try to solve the error! \n")
            tmp <- try( ElasticSCADSVM <- penalizedSVM::scad_L2.svc(lambda1 = lambda1_new, lambda2=lambda2,
                                                  x=df_x_as_matrix1, y=df_y_as_matrix,
                                                  a = 3.7, maxIter = 100000,
                                                  verbose = FALSE, tol = 10^(-5)), silent = TRUE)
            count <- count+1
            df_x_as_matrix1 <- df_x_as_matrix[,column_start:ncol(df_x_as_matrix)]
    ...
    #sep = df_x_as_matrix[,ElasticSCADSVM$xind, drop=FALSE] %*% ElasticSCADSVM$w + ElasticSCADSVM$b
    ...
    if (ElasticSCADSVM[1] == "No variable selected."){
      ...
      #as.data.frame(as.numeric(rowSums(df1[names(ElasticSCADSVM$w)] * ElasticSCADSVM$w))+ElasticSCADSVM$b)
      z_hat = as.data.frame(ElasticSCADSVM$fitted)

      #populate betas_hat
      betas_hat[which(names(betas_hat) %in% attr(ElasticSCADSVM$w, "names"))]=ElasticSCADSVM$w
    }
    ...
    c <- sum(OptimalCutpoints::optimal.cutpoints(data= df_hat, X = "z_hat", status = "y", methods = "MCT", tag.healthy =0,
      ...
      length(OptimalCutpoints::optimal.cutpoints(data= df_hat, X = "z_hat", status = "y", methods = "MCT", tag.healthy =0,
                                                 generalized.Youden=F)$MCT$Global$optimal.cutoff$cutoff)
    #y_hat
    #test = predict(ElasticSCADSVM, df_x_as_matrix, lambda1)
    y_hat = ifelse(df_hat["z_hat"] < c, 0, 1)
    colnames(y_hat)="y_hat"
    df_hat = cbind(df_hat, y_hat)
  ...
  } else if (model_type=="l1SVM"){
    #install.packages("sparseSVM")
    #library(sparseSVM)
    #https://cran.r-project.org/web/packages/sparseSVM/sparseSVM.pdf

    l1SVM <- sparseSVM::sparseSVM(X=df_x_as_matrix, y=df_y_as_matrix, alpha = 1,
                                  gamma = 0.1, lambda=if(lambda1==1) {c(2, lambda1)} else {c(1, lambda1)}, #c(1,lambda1)
    ...
    if(l1SVM$levels[1] =="-1"){
      ##ATTENTION: I invert the signs since in the original package (sparseSVM) the model predicts -1 and not 1!
      z_hat = as.data.frame(-stats::coef(l1SVM, lambda1, exact = TRUE)[1] + (df_x_as_matrix %*% -stats::coef(l1SVM, lambda1, exact = TRUE)[-1])) # coef(l1SVM, lambda1, exact = TRUE)[1])
    } else {
    ...
    #after many controls, I understood that the estimation is biased in centering the estimation in 0. This produces bad
    #performances in CC and other measures that depend on the cut-off even if the AUC is good!
    ...
    #y_hat
    y_hat = ifelse(df_hat["z_hat"] < c, 0, 1)
    #y_hat1 = ifelse(predict(l1SVM, df_x_as_matrix, lambda1) =="-1", 0 , 1)
    #cbind(y_hat, y_hat1, df_hat["y"])
    colnames(y_hat)="y_hat"
    df_hat = cbind(df_hat, y_hat)
    #test = predict(l1SVM, df_x_as_matrix, lambda1)
    #y_hat==test
    #df_hat[,c("y","y_hat")]
    #z_hat1 = sweep(df_x_as_matrix %*% stats::coef(l1SVM, lambda1, exact = TRUE)[-1], 2,  stats::coef(l1SVM, lambda1, exact = TRUE)[1], "+")
    #y_hat1 = ifelse(z_hat1 > 0, -1, 1)
    #test = predict(l1SVM, df_x_as_matrix, lambda1)
    #y_hat==test
    #y_hat1==test
    #y_hat==y_hat1
    #z_hat==z_hat1
    #cbind(z_hat, z_hat1, z_hat==z_hat1, y_hat, y_hat1, y_hat==y_hat1, test, y_hat==test)

    #build betas_hat properly

    if(l1SVM$levels[1] =="-1"){
      betas_hat = -stats::coef(l1SVM, lambda1, exact = TRUE)[-1] #at this moment I dont save the constant
    } else {
      betas_hat = stats::coef(l1SVM, lambda1, exact = TRUE)[-1] #at this moment I dont save the constant
    }
    #prepare the output
  ...
  } else if (model_type=="enSVM"){
    #install.packages("sparseSVM")
    #library(sparseSVM)
    #https://cran.r-project.org/web/packages/sparseSVM/sparseSVM.pdf

    enSVM <- sparseSVM::sparseSVM(X=df_x_as_matrix, y=df_y_as_matrix, alpha = alpha,
                                  gamma = 0.1, lambda= if(lambda1==1) {c(2, lambda1)} else {c(1, lambda1)}, #c(1,lambda1),
                                  preprocess = "none",# c("standardize", "rescale", "none"),
                                  screen = "SR", # c("ASR", "SR", "none"),
                                  max.iter = 100000, eps = 1e-7,
    ...
    if(enSVM$levels[1] =="-1"){
      ##ATTENTION: I invert the signs since in the original package (sparseSVM) the model predicts -1 and not 1!
      z_hat = as.data.frame(-stats::coef(enSVM, lambda1, exact = TRUE)[1] + (df_x_as_matrix %*% -stats::coef(enSVM, lambda1, exact = TRUE)[-1]))
    } else {
      #if the predicted value is 1 we apply the normal calculation
      z_hat = as.data.frame(stats::coef(enSVM, lambda1, exact = TRUE)[1] + (df_x_as_matrix %*% stats::coef(enSVM, lambda1, exact = TRUE)[-1])) # coef(enSVM, lambda1, exact = TRUE)[1])
    }
    colnames(z_hat)="z_hat"
    df_hat = cbind(df1, z_hat)
    ...
    #y_hat
    y_hat = ifelse(df_hat["z_hat"] < c, 0, 1)
    #z_hat1 = sweep(df_x_as_matrix %*% coef(enSVM, lambda1, exact = TRUE)[-1], 2,  coef(enSVM, lambda1, exact = TRUE)[1], "+")
    #y_hat1 = ifelse(z_hat1 > 0, -1, 1)
    #test = predict(enSVM, df_x_as_matrix, lambda1)
    #y_hat==test
    #y_hat1==test
    #y_hat==y_hat1
    #z_hat==z_hat1
    ...
    #build betas_hat properly
    if(enSVM$levels[1] =="-1"){
      betas_hat = -stats::coef(enSVM, lambda1, exact = TRUE)[-1] #at this moment I dont save the constant
    } else {
      betas_hat = stats::coef(enSVM, lambda1, exact = TRUE)[-1] #at this moment I dont save the constant
    }
  ...
  #measures
  #optimal measures
  optimal <- OptimalCutpoints::optimal.cutpoints(data= df_hat, X = "z_hat", status = "y", methods = "Youden", tag.healthy =0, generalized.Youden=F)

  #AUC e YI
  auc <-  optimal$Youden$Global$measures.acc$AUC[1]
  youden_index <- optimal$Youden$Global$optimal.criterion[1]

  #this is the youden index if we do not use as threshould 0
  potential_youden_index = optimal$Youden$Global$optimal.criterion

  #potential cut-off
  potential_cutoff = optimal$Youden$Global$optimal.cutoff$cutoff
  ...
  spec <- TN/(TN+FP)
  fnr <- FN/(FN+TP)
  #youden index at the 0 threshold
  ...
  if ((trace  %in% c(1,2))) {
    ...
    if (length(lambda1)!=0) {cat("lambda:", lambda1, "; ")}
    if (!is.na(cost)) {cat("cost:", cost, "; \n")}
  ...
  return(list(model=model,
              ...
              corrclass=corrclass,
              auc=auc,
              potential_youden_index=potential_youden_index,
              lambda1=lambda1, lambda2=lambda2,
              alpha=alpha, scale=scale, cost=cost,
...
#'
#' @return a list containing the prediction using the SVM methods and the value
#' of the main accuracy measure.
#'
#' @examples
#' library(pye)
...
#'
#' @export
all_svm_predict <- function (df, X=names(df[,!(names(df) == y)]), y="y", model_to_use, fold=NULL,
  ...
  #model_type can be: "SVM_linear", "SVM_polynomial", "SVM_radial",
  #"SVM_sigmoid", "SCADSVM", "ElasticSCADSVM", "l1SVM", "enSVM"
  if (model_to_use$model_type %in% c("SVM_linear", "SVM_polynomial", "SVM_radial", "SVM_sigmoid")){

    #predict the svm_linear model
    z_hat <- as.data.frame(attr(stats::predict(object=model_to_use$model, newdata=df_x_as_matrix, decision.values = TRUE), "decision.values"))
    colnames(z_hat)="z_hat"
    df_hat = cbind(df1, z_hat)
  ...
  } else if (model_to_use$model_type %in% c("SCADSVM", "ElasticSCADSVM", "l1SVM", "enSVM")){

    if (model_to_use$model_type %in% c("SCADSVM", "ElasticSCADSVM")){
    ...
    } else if (model_to_use$model_type %in% c("l1SVM", "enSVM")){
      ##predict

      #if the predicted value is -1
      if(model_to_use$model$levels[1] =="-1"){
        ##ATTENTION: I invert the signs since in the original package (sparseSVM) the model predicts -1 and not 1!
        z_hat = as.data.frame(-stats::coef(model_to_use$model, model_to_use$lambda1, exact = TRUE)[1] + df_x_as_matrix %*% -stats::coef(model_to_use$model, model_to_use$lambda1, exact = TRUE)[-1])
      } else {
        #if the predicted value is 1 we apply the normal calculation
        z_hat = as.data.frame(stats::coef(model_to_use$model, model_to_use$lambda1, exact = TRUE)[1] + df_x_as_matrix %*% stats::coef(model_to_use$model, model_to_use$lambda1, exact = TRUE)[-1])
      }
    ...
    #this is the youden index if we do not use as threshold 0
    potential_youden_index = optimal$Youden$Global$optimal.criterion
  ...
  } else {stop("The parameter model_to_use$model_type is not valid!")}

  if (trace %in% c(1,2)) {
    #print the results only if c_function_of_covariates = FALSE
    if(c_function_of_covariates==FALSE){
      cat("-> Predictions made using model:", model_to_use$model_type, "; \n")
	  cat("->")
      if (length(fold)!=0) {cat("fold =", fold, "; ")}
      if (length(model_to_use$lambda1)!=0) {cat("lambda:", model_to_use$lambda1, "; ")}
      if (!is.na(model_to_use$cost)) {cat("cost:", model_to_use$cost, "; \n")}
      cat("youden_index:", youden_index, "; sensitivity:", sensitivity, "; specificity:", spec, "; geometric_mean:", gm, "; fdr:", fdr, "; mcc:", mcc, "; auc:", auc, "; corrclass:", corrclass, "; \n")
      cat("TP:", TP, "; TN:", TN, "; FP:", FP, "; FN:", FN, "; betas_hat: \n")
  ...
  return(list(model=model_to_use$model, model_type=model_to_use$model_type, betas_hat=model_to_use$betas_hat,
              ...
              mcc=mcc, corrclass=corrclass, auc=auc, potential_youden_index=potential_youden_index,
              lambda1=model_to_use$lambda1, lambda2=model_to_use$lambda2, alpha=model_to_use$alpha,
              scale=model_to_use$scale, cost=model_to_use$cost, c_hat=model_to_use$c_hat, z_hat=z_hat,
              y_hat=y_hat, n_total_var=n_total_var, n_predicted_zeros=n_predicted_zeros,
              n_predicted_non_zeros=n_predicted_non_zeros, n_caught_betas= n_caught_betas,
              n_non_caught_betas=n_non_caught_betas, n_caught_zero=n_caught_zero,
...
svm.cv <- function (df, X, y, C, lambda, tau, alpha_g, penalty_g, folds_i, k, regressors_betas,
                     trace, model_type, kernel_g, a1_g, a2_g, trend_g, gamma_start_input, gamma_start_default,
                     regressors_gammas, max_iter_g, delta_g, max_alpha_g, stepsizeShrink_g, min_alpha_g,
                     convergence_error_g,
  ...
  if (model_type %in% c("SCADSVM", "ElasticSCADSVM", "l1SVM", "enSVM")) {
    ...
    if (used_cores > 1) {
      ...
      cl <- parallel::makeCluster(used_cores, outfile="log_SVM_models.txt", setup_strategy = "sequential")
      ...
      fitted_models <- parallel::parLapply(cl, lambda, function(x) all_svm_estimation(df=train_df1, X=X, y=y,
                                                                                      lambda1=x, lambda2=0.05,
                                                                                      alpha=0.5,
      ...
      fitted_models <- lapply(lambda, function(x) all_svm_estimation(df=train_df1, X=X, y=y,
                                                                     lambda1=x, lambda2=0.05,
                                                                     alpha=0.5,
                                                                     regressors_betas=regressors_betas,
                                                                     model_type=model_type,
  ...
  all_measures_test <- mapply(function(z) all_svm_predict(df=test_df, X=X,
                                                          model_to_use = fitted_models[[z]], fold=k, trace=trace,
                                                          c_function_of_covariates=c_function_of_covariates),
                              1:length(lambda))

  #then I have to evaluate the different taus with all_svm_predict and choose the best tau
  ...
  return(methods::new("svm_cross_validation_output", auc=auc, aauc=aauc, aYI=aYI, youden_index=youden_index,
...
#' possibly gammas) using svm (and possibly covYI).
#'
...
#' 0:visualize nothing. Default is 1
#' @param model_type model to use in the estimation. The user can choose
#' among "SCADSVM", "ElasticSCADSVM", "l1SVM", "enSVM".
#' @param seed fix the seed. Default is 1
...
#'
#'   #competitors svm
#'   svm_models = "SCADSVM" #c("SCADSVM", "ElasticSCADSVM", "l1SVM", "enSVM")
#'
#'   for (m in svm_models){
#'
...
#'       return(all_svm_estimation(df=df, X=X, y=y, lambda1=lambda,
#'		 lambda2=0.05, alpha=0.5, regressors_betas=regressors_betas, model_type=m,
#' 		 trace=1))
#'     }
...
#'       z_hat <- all_svm_estimation(df=df, X=X, y=y, lambda1=lambda_star,
#'			lambda2=0.05, alpha=0.5, regressors_betas=regressors_betas, model_type=m,
#'			trace=1)$z_hat$z_hat
#'       wrapper_tau <- function(tau){
#'         return(covYI_KS_estimation(df=cbind(df, z_hat=z_hat), z="z_hat",
#'			y=y, C=C, tau=tau, gamma_start_input=NULL,
...
#'     assign(name, svm_compute_cv(model_type = m, trace=1, n_folds=n_folds,
...
#'
#' @export
svm_compute_cv <- function (n_folds, df, X=names(df[,!(names(df) %in% c(y,C))]), y="y", C=NULL, lambda,
  ...
  #fill the matrices
  results <- mapply(function(k) svm.cv(df=df1[,names(df1)!="ID"], X=X, y=y, C=C, lambda=lambda, tau=tau,
                                        alpha_g=alpha_g, penalty_g=penalty_g, folds_i=folds_i, k=k,
                                        regressors_betas=regressors_betas, trace=trace, model_type=model_type,
  ...
  if (trace %in% c(1,2)) {
	cat("----------------> END OF THE CROSS-VALIDATION OF THE PENALIZED SVM METHODS <----------------- \n")
