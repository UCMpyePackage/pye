...
longpye_KS <- function(df_train, df_test=NULL, id="id", X=names(df_train[,!(names(df_train) %in% c(y,id,t))]),
  ...
  if (!is.matrix(betas)){
    if (!is.vector(betas)){
      betas <- matrix(betas, nrow=length(X), ncol=length(unique(df1[,"t"])))
      rownames(betas) <- X
      colnames(betas) <- order(unique(df1[,"t"]))
    }
  }
  ...
  if (length(lambda)>1){stop("longpye_KS stopped becuase lambda needs to have length 1")}
  ...
  if (length(betas) != length(X)*length(unique(df1$t))){
  ...
  z_y0_i <- lapply(1:n_times, function (i) stats::setNames(transform(merge(df1["id"],
    as.matrix(df1[df1$t == i & df1[,y] == 0, names(df1) %in% X, drop=FALSE])%*%betas[,i], by=0), row.names=Row.names, Row.names=NULL), c("id", paste0("z_", i, "_hat"))))
  z_y1_i <- lapply(1:n_times, function (i) stats::setNames(transform(merge(df1["id"],
    as.matrix(df1[df1$t == i & df1[,y] == 1, names(df1) %in% X, drop=FALSE])%*%betas[,i], by=0), row.names=Row.names, Row.names=NULL), c("id", paste0("z_", i, "_hat"))))

  #append the z(t)
  z_i <- lapply(1:n_times, function (i) stats::setNames(transform(merge(df1["id"],
              as.matrix(df1[df1$t == i, names(df1) %in% X, drop=FALSE])%*%betas[,i], by=0), row.names=Row.names), c("ID_rows", "id", "z_i_hat")))
  ...
  p_grad0_normal <- t(apply(t0, 1, numDeriv::grad, func = func0))
  #p_grad0_normal_betas <- mnormt::dmnorm(t0, rep(0, ncol(t0)), varcov = diag(1, ncol(t0)))
  gr0_betas_i <- t(sapply(list_of_features_y0, function(x) {
                          colSums(p_grad0_normal * -(as.matrix(x[!(names(x) %in% "id")]) %*% H0_inv)) / (nrow(t0) * H0_det)
                          #mean(colSums(p_grad0_normal * -(as.matrix(x[!(names(x) %in% "id")]) %*% H0_inv)) / (nrow(t0) * H0_det))
                          #sum((p_grad0_normal_betas %*% -(as.matrix(x[!(names(x) %in% "id")]) %*% H0_inv)) / (nrow(t0) * H0_det))
                        }))
  rownames(gr0_betas_i) <- X
  colnames(gr0_betas_i) <- order(unique(df1[,"t"]))

  #func0 <- function(z) {mnormt::pmnorm(z, rep(0, ncol(t0)), varcov = diag(1, ncol(t0))) }
  #p_grad0_normal_c <- t(apply(t0, 1, numDeriv::grad, func = func0))
  gr0_c_i <- (colSums(p_grad0_normal) %*% H0_inv) / (nrow(t0) * H0_det)
  colnames(gr0_c_i) <- names(c)

  #cases
  #New:
  func1 <- function(z) {mnormt::pmnorm(z, rep(0, ncol(t1)), varcov = diag(1, ncol(t1))) }
  p_grad1_normal <- t(apply(t1, 1, numDeriv::grad, func = func1))
  #p_grad1_normal_betas <- mnormt::dmnorm(t1, rep(0, ncol(t1)), varcov = diag(1, ncol(t1)))
  gr1_betas_i <- t(sapply(list_of_features_y1, function(x) {
                          colSums(p_grad1_normal * -(as.matrix(x[!(names(x) %in% "id")]) %*% H1_inv)) / (nrow(t1) * H1_det)
                          #mean(colSums(p_grad1_normal * -(as.matrix(x[!(names(x) %in% "id")]) %*% H1_inv)) / (nrow(t1) * H1_det))
                          #sum((p_grad1_normal_betas %*% -(as.matrix(x[!(names(x) %in% "id")]) %*% H1_inv)) / (nrow(t1) * H1_det))
                        }))
  rownames(gr1_betas_i) <- X
  colnames(gr1_betas_i) <- order(unique(df1[,"t"]))

  #func1 <- function(z) {mnormt::pmnorm(z, rep(0, ncol(t1)), varcov = diag(1, ncol(t1))) }
  #p_grad1_normal_c <- t(apply(t1, 1, numDeriv::grad, func = func1))
  gr1_c_i <- (colSums(p_grad1_normal) %*% H1_inv) / (nrow(t1) * H1_det)
  ...
  if (length(df_test) > 0) {
    ...
    z_i_test <- lapply(1:n_times, function (i) stats::setNames(transform(merge(df1_test["id"],
                    as.matrix(df1_test[df1_test$t == i, names(df1_test) %in% X, drop=FALSE])%*%betas[,i], by=0), row.names=Row.names, Row.names=NULL), c("id", paste0("z_", i, "_hat"))))
...
longpye_KS_estimation <- function(df, id="id", X=names(df[,!(names(df) %in% c(y,id,t))]), y="y",
  ...
  if (length(lambda)>1){stop("longpye_KS_estimation stopped becuase lambda needs to have length 1")}
  if (!(penalty %in% c("L12","L1","EN","SCAD","MCP"))){stop("A wrong value has been assigned to the parameter penalty.")}
  ...
  betas1_initial_zeros <- matrix(0, nrow =length(X), ncol = length(unique(df1[,"t"])))
  ...
  if (length(beta_start_input)==0){
    if (beta_start_default=="zeros"){
      ...
      rownames(betas_start) <- X
      colnames(betas_start) <- order(unique(df1[,"t"]))
      betas1 <- betas_start

    } else if (beta_start_default=="corr") {

      betas1 <- matrix(0, nrow =length(X), ncol = length(unique(df1[,"t"])))
      rownames(betas1) <- X
      colnames(betas1) <- order(unique(df1[,"t"]))
      for (tt in unique(df1[,"t"])){
        #compute the corr between every x and y
        corr.xy <- data.frame(matrix(ncol = length(X), nrow = 0))
        names(corr.xy) <- X
        corr.xy[1:3,] <- t(sapply(c("pearson", "kendall", "spearman"), function (h) unlist(lapply(X, function (x) stats::cor(df1$y[df1$t==tt], df1[df1$t==tt,x],  method = h)))))
        ...
        betas1_initial_corr[is.na(betas1_initial_corr)] <- 0
        betas_start <- betas1_initial_corr
        names(betas_start) <- X
        betas1[,tt] <- betas_start
      }

    } else {stop("The parameter beta_start_default can only be equal to 'zeros', 'corr'.")}
  } else {
    if (nrow(beta_start_input) == length(X) & ncol(beta_start_input) == length(unique(df1[,"t"]))){
  ...
  c_start <- c
  #add c to the betas
  betas1 <- rbind(betas1[!(rownames(betas1) %in% "c"), ], c=c) #merge betas1 and c
  ...
  #wrappers
  delta_fx <- function(x){
    x <- matrix(x, nrow=length(X)+1, ncol=length(unique(df1[,"t"])))
    rownames(x) <- c(X,"c")
    colnames(x) <- order(unique(df1[,"t"]))
    execution <- longpye_KS(df_train=df1[,names(df1)!="ID_rows", drop=FALSE], id="id", X=X[X %in% rownames(x)], t="t",
                            y=y, betas=x[1:length(X),], lambda=lambda, c=x[length(X)+1,],
    ...
    #result <- c(-as.vector(execution$gr_yi_betas), -execution$gr_yi_c)
    result <- as.vector(rbind(-execution$gr_yi_betas, c=-execution$gr_yi_c))
    comb <- expand.grid(c(X,"c"), order(unique(df1[,"t"])))
    names(result) <- paste(comb$Var1, comb$Var2, sep = "_")
    #print(result)
    return(result)
  }

  proxx <- function(x, eta){
    #c_pos <- seq(from=which(rownames(betas1)=="c"), to=which(rownames(betas1)=="c")*length(unique(df1[,"t"])) , by=which(rownames(betas1)=="c"))
    #we have to apply the penalty to all the betas of a variable in the different times together
    #to allow the "all or nothing" criteria to improve interpretability of the result
    result <- matrix(x, nrow=length(X)+1, ncol=length(unique(df1[,"t"])))
    rownames(result) <- c(X,"c")
    colnames(result) <- order(unique(df1[,"t"]))
    x2 <- rowMeans(abs(result))
    temp <- prox_penalty(betas=x2[-length(x2)], lambda=eta*lambda, alpha=alpha, a=a)
    #result <- c(prox_penalty(betas=x[-c_pos], lambda=eta*lambda, alpha=alpha, a=a), x[c_pos])
    result[rownames(result) %in% names(temp[temp==0]),] <- 0
  ...
  Fx <- function(x){
    ...
    rownames(x) <- c(X,"c")
    colnames(x) <- order(unique(df1[,"t"]))
    result <- -getElement(longpye_KS(df_train=df1[,names(df1)!="ID_rows", drop=FALSE], id="id",  X=X[X %in% rownames(x)],
                                     t="t", y=y, betas=x[1:length(X),], lambda=lambda, c=x[length(X)+1,], kernel=kernel,
                                     alpha=alpha, a1=a1, a2=a2, penalty=penalty),  paste0("longpye_", penalty)) #"youden_index")
    return(as.vector(result))
  }

  c_pos <- seq(from=which(rownames(betas1)=="c"), to=which(rownames(betas1)=="c")*length(unique(df1[,"t"])) , by=which(rownames(betas1)=="c"))
  x0 <- as.vector(betas1)
  comb <- expand.grid(rownames(betas1), colnames(betas1))
  names(x0) <- paste(comb$Var1, comb$Var2, sep = "_")
  if (trend == "monotone"){
    estim <- mmAPG(x0=x0, c_pos=c_pos, delta_fx=delta_fx, proxx=proxx, Fx=Fx, lambda=lambda, penalty=penalty,
    ...
    estim <- mnmAPG(x0=x0, c_pos=c_pos, delta_fx=delta_fx, proxx=proxx, Fx=Fx, lambda=lambda, penalty=penalty,
                                      fold=fold, stepsizeShrink=stepsizeShrink, delta=delta, max_alpha=max_alpha,
                                      max_iter=max_iter, min_alpha=min_alpha, convergence_error=convergence_error,
                                      trace=trace, seed=seed, max.print=max.print,
                                      zeros_stay_zeros_from_iteration=zeros_stay_zeros_from_iteration)
  ...
  #divide betas1 and c
  betas_hat <- estim$x1[-c_pos]
  c_hat <- estim$x1[c_pos]

  betas_hat <- matrix(betas_hat, nrow=length(X), ncol=length(unique(df1[,"t"])))
  rownames(betas_hat) <- X
  colnames(betas_hat) <- order(unique(df1[,"t"]))

  if(lambda == 1234){
    betas_hat <- matrix(0, nrow =length(X), ncol = length(unique(df1[,"t"])))
    rownames(betas_hat) <- X
    colnames(betas_hat) <- order(unique(df1[,"t"]))
    for (tt in unique(df1[,"t"])){
      #compute the corr between every x and y
      ...
      #mean of the corrs and sort names
      mean.corr.xy <- as.data.frame(t(colMeans(corr.xy)))
      betas_hat_initial_corr <- as.numeric(mean.corr.xy)
      betas_hat_initial_corr[is.na(betas_hat_initial_corr)] <- 0
      betas_start <- betas_hat_initial_corr
      names(betas_start) <- X
      betas_hat[,tt] <- betas_start
    }
    c_hat <- c_start
  }

  #test
  #longpye_KS_value1 <- longpye_KS(df_train=df1[,names(df1)!="ID_rows", drop=FALSE],
  ...
  #compute z_hat
  longpye_KS_value <- longpye_KS(df_train=df1[,names(df1)!="ID_rows", drop=FALSE],
                                 df_test=df1[,names(df1)!="ID_rows", drop=FALSE],
...
subset_longpye_KS <- function(df_train, df_test, X, y, id, t, betas, lambda, c, fold, alpha, trace, a1, a2, penalty,
                           cv_time, niter, kernel, c_hat_discriminant){

  longpye_KS_result <- longpye_KS(df_train=df_train[,names(df_train)!="ID_rows"], df_test=df_test[,names(df_test)!="ID_rows"],
  ...
  if (trace %in% c(1,2)) {
    ...
    if (length(fold)!=0) {cat("fold =", fold, "; \n")}
    betas1 <- as.vector(betas)
    comb <- expand.grid(rownames(betas), colnames(betas))
    names(betas1) <- paste(comb$Var1, comb$Var2, sep = "_")
    visualize_betas <- c(betas1[which(betas1!=0)], c_hat)
    cat("lambda:", lambda, "; penalty:", penalty, "; longpye_KS:", getElement(longpye_KS_result,  paste0("longpye_",penalty)), "; youden_index:", longpye_KS_result$youden_index_test, "; sensitivity:", longpye_KS_result$sensitivity_test, "; specificity:", longpye_KS_result$specificity_test, "; geometric_mean:", longpye_KS_result$geometric_mean_test, "; fdr:", longpye_KS_result$fdr_test, "; mcc:", longpye_KS_result$mcc_test, "; auc:", longpye_KS_result$auc_test, "; corrclass:", longpye_KS_result$corrclass_test, " \n")
...
#' @importFrom stats setNames
longpye_KS.cv <- function (df, X, y, C, id, t, lambda, trace=1, alpha, a1, a2, penalty, folds_i, k,
  ...
  if (used_cores > 1) {
    ...
    fitted_models <- parallel::parLapply(cl, lambda, function(x) longpye_KS_estimation(df=train_df[,!(names(train_df) %in% "ID_rows")],
    ...
    fitted_models <- lapply(lambda, function(x) longpye_KS_estimation(df=train_df[,!(names(train_df) %in% "ID_rows")],
                                                                   X=X, y=y, lambda=x, id=id, t="t",
                                                                   beta_start_input=beta_start_input,
                                                                   beta_start_default=beta_start_default,
                                                                   trace=trace,
  ...
  all_measures_test <- mapply(function(x, xx, xxx, z, zz) subset_longpye_KS(df_train=train_df, df_test=test_df,
...
longpye_KS_compute_cv <- function (n_folds, df, X=names(df[,!(names(df) %in% c(y,C))]), y="y",
                                ...
                                max_iter=10, #reduced for the CV
                                min_alpha=1e-7,
  ...
  #fill the matrices
  results <- mapply(function(k) longpye_KS.cv(df=df1[,names(df1)!="ID_rows"], X=X, y=y, id="id", t="t",
