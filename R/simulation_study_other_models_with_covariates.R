#' @title model_simulation_study_real_data
#'
...
#' estimating the considered competitor method of pye (penalized
#' logistic regressions, penalized SVM and penalized AUC-based
#' method), in the train and evaluating the classification
...
#' @param model_estimation_function function to use for the estimation. The user
#' can choose among "glmnet_estimation", "all_svm_estimation", "AucPR_estimation"
#' @param model_prediction_function function to use for the prediction. The user
...
#'   #                                  START                                  |
#'   #--------------------------------------------------------------------------
#'   other_models_vector = c("logLasso", "SCADSVM", "AucPR_L1") # c("logLasso",
#' 						# "logElasticNet", "logSCAD", "logMCP",
...
#'       cat("---> Starting with measure", measures[mm], ", for the model", m, "<---\n")
...
#'         tau_to_use <- 0
#'       }
#'       assign(name, model_simulation_study_real_data(n=n, df=df, X=X, y=y, C=C,
...
#' 			max_iter_g=10 #because it is an example. Please use more iterations
#' 					   ))
#'
#'       cat("Next measure!")
#'     }
...
#' @importFrom pROC roc
#' @export
model_simulation_study_real_data <- function(n=1000, df, X=names(df[,!(names(df) %in% c(y,C))]), y="y", C=NULL,
                                             model_estimation_function, model_prediction_function,
                                             model_type, lambda, tau=0, trace=2, alpha=0.5, regressors_betas=NULL,
  ...
  if (length(tau)>1){stop("model_simulation_study_real_data stopped because tau needs to have length 1")}
  if (length(lambda)>1){stop("model_simulation_study_real_data stopped because lambda needs to have length 1")}
  ...
  func <- function(seed, n, df, X, y, C, lambda, tau,
    ...
    #split <- sample(rep(1:nrow(df)), size=(round(nrow(df)*0.7,0)), replace=FALSE)
    df_sort <- df[order(getElement(df, y)),1:2]
    split_0 <- sample(rep(1:nrow(df_sort[getElement(df_sort, y)==0,])), size=(round(nrow(df_sort[getElement(df_sort, y)==0,])*0.7,0)), replace=FALSE)
    split_1 <- nrow(df_sort[getElement(df_sort, y)==0,]) + sample(rep(1:nrow(df_sort[getElement(df_sort, y)==1,])), size=(round(nrow(df_sort[getElement(df_sort, y)==1,])*0.7,0)), replace=FALSE)
    split <- sort(c(split_0, split_1))
    train_df <- df[which(df$ID %in% df_sort[split, "ID"]),]
    test_df <- df[which(df$ID %in% df_sort[-split, "ID"]),]

    #train PYE KS
    train_solution <- model_estimation_function(df=train_df[,names(train_df)!="ID"], X=X, y=y,
                                                lambda=lambda,
                                                cost=lambda, lambda1=lambda, alpha=alpha,
                                                regressors_betas=regressors_betas,
                                                model_type=model_type, trace=trace)
    ...
    #test the results
    test_solution <- model_prediction_function(df=test_df[,names(test_df)!="ID"], X=X, y=y,
                                               model_to_use = train_solution, trace=trace)
  ...
  if (trace %in% c(1,2)){
    cat("------------------------------------------------------------------\n|            starting simulation study with", n, "simulations          |\n------------------------------------------------------------------\n")
  }
  ...
  if (used_cores > 1) {
    ...
    cl <- parallel::makeCluster(used_cores, outfile="log_sim_Other_real.txt")
    ...
    parallel::clusterExport(cl, c("n", "df1", "X", "y", "C", "lambda", "seeds", "tau", "model_estimation_function",
                                  "model_prediction_function", "trace", "model_type", "alpha", "a1_g", "a2_g",
  ...
  betas_times_selected <- rowSums(sapply(c(1:length(seeds)), function(x) getElement(simulation_study[[x]]$train_solution, "betas_hat"))!=0)
  ...
  betas <- lapply(c(1:length(seeds)), function(k) getElement(simulation_study[[k]]$train_solution, "betas_hat"))
  ...
  results <- list(model_type=model_type, simulation_time=simulation_time,
...
#' @title model_simulation_study_synthetic_data
#'
#' @description function to perform the simulation on n different
...
model_simulation_study_synthetic_data <- function(n=1000, rows_train=50, rows_test=1000, cols=2000, cols_cov=20,
                                                  ...
                                                  lambda, tau=0, trace=2, alpha=0.5, used_cores=1,
                                                  c_function_of_covariates=FALSE,
                                                  alpha_g=0.5, penalty_g="L1", kernel_g="gaussian", a1_g=3.7, a2_g=3,
                                                  trend_g="monotone", gamma_start_input=NULL, gamma_start_default="zeros",
  ...
  #starting controls
  if (length(tau)>1){stop("model_simulation_study_synthetic_data stopped because tau needs to have length 1")}
  if (length(lambda)>1){stop("model_simulation_study_synthetic_data stopped because lambda needs to have length 1")}
  if (!(penalty_g %in% c("L12","L1","EN","SCAD","MCP"))){stop("A wrong value has been assigned to the parameter penalty.")}
  if (!(trace %in% c(0,1,2))){stop("The parameter trace has been wrongly assigned. It can be 0 (no print),1 (partial print) or 2 (full print).")}
  if (n < 2){stop("n needs to be at least 2.")}
  ...
  #function to be computed
  func <- function(seed, n, rows_train, rows_test, cols, cols_cov, covar, lambda, tau,
                   model_estimation_function, model_prediction_function,
    ...
    train_solution <- model_estimation_function(df=train_df, X=X, y=y,
                                                lambda=lambda, cost=lambda, lambda1=lambda,
    ...
    if(c_function_of_covariates == TRUE) {
      ...
      train_covYI_solution <- covYI_KS_estimation(df=cbind(train_df[,names(train_df)!="ID"],
    ...
    #ROC curve @ certain levels
    est_roc <-  pROC::roc(as.numeric(getElement(train_df, y)), z_hat[,"z_hat"], levels=c(0, 1), direction="<")
    roc_spec_points <- seq(0, 1, by = 0.1)
    roc_sens_points_train <- pROC::coords(est_roc, 1 - roc_spec_points, input="specificity", ret="sensitivity", transpose=TRUE)

    #test the results
    test_solution <- model_prediction_function(df=test_df, X=X, y=y,
                                               model_to_use = train_solution, trace=trace)
  ...
  if (used_cores > 1) {
    ...
    cl <- parallel::makeCluster(used_cores, outfile="log_sim_Other_synthetic.txt")
    if (!("cluster" %in% class(cl)))
      stop("cl is not of class 'cl'; see ?makeCluster")

    cat("Start parallel computing for the simulation, I am using:", length(cl),"cores \n")
    parallel::clusterExport(cl, c("n", "rows_train", "rows_test", "cols", "cols_cov", "covar", "lambda", "seeds", "tau",
                                  "model_estimation_function", "model_prediction_function", "trace", "model_type",
                                  "alpha", "alpha_g", "penalty_g", "a1_g", "a2_g",
                                  "c_function_of_covariates", "run_aauc"), envir = environment())
    parallel::clusterCall(cl, function() require(pye))
